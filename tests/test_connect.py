...
def test_get_connection_file(tmp_path):
    cfg = Config()
    cfg.ProfileDir.location = str(tmp_path)
    cf = "kernel.json"
...
def test_get_connection_info(tmp_path):
    cf = str(tmp_path.joinpath("kernel.json").resolve())
...
def test_port_bind_failure_raises(request, tmp_path):
    cfg = Config()
    cfg.ProfileDir.location = str(tmp_path)
    cf = "kernel.json"
    app = DummyKernelApp(config=cfg, connection_file=cf)
...
def test_port_bind_failure_gives_up_retries(request, tracemalloc_resource_warning, tmp_path):
    cfg = Config()
    cfg.ProfileDir.location = str(tmp_path)
    cf = "kernel.json"
    app = DummyKernelApp(config=cfg, connection_file=cf)
